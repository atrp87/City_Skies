...
// ? Handle Errors
// ? unit system calls
// ? ADD URL REQUEST OBJECT
// ? add and remove renders on new request + cta prompts
// ! CSS BEFORE AFTER & AFTER &DEG all class list C / F CONTENT CHANGE ON CLICK
...
//   icons: ``,
//   revGeo: ``
// key: '8dd55712ad3e5e950fb94620922f7ada'
// }
...
scaleBtn.forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    if (buttonTxt.includes('Metric')) {
      console.log('Metric');
      // fetchWeatherData() // metric
    } else if (buttonTxt.includes('Imperial')) {
      console.log('Imperial');
      // fetchWeatherData() //Imperial
    };
  });
});
...
submitBtn.addEventListener("click", () => {
  if (input.value === '') {
    cta.innerHTML = 'City Name Required'
  } else {
    fetchWeatherData(input.value.toLowerCase())
    input.value = '';
  }
...
const fetchWeatherData = async (cityName) => {
  ...
  try {
    ...
    renderError(`Unable to complete request, ${err.message}`)
...
window.addEventListener('load', async () => {
  try {
    ...
    console.log(dataGeo.address.town);
